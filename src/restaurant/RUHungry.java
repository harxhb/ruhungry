...
public class RUHungry {
    ...
    public void menu(String inputFile) {
        ...
        int bron = StdIn.readInt(); //reads number of categories (6)
        StdIn.readLine(); //reads line^
        categoryVar = new String[bron]; //assigns length to  String array where each index corresponds to parallel category from menuVar
        menuVar = new MenuNode[bron]; //assigns length to hashtable where each index is sorted by category and contains a linked list of menuNodes

        for(int i = 0; i < bron; i++) 
        {
            categoryVar[i] = StdIn.readLine(); //assigns category to said index (ex. appetizer)
            int stephSize = StdIn.readInt(); //assigns # of dishes in said category (ex. 4)
            StdIn.readLine(); //reads line^
            for (int j = 0; j < stephSize; j++) {
                String n = StdIn.readLine(); //reads name of dish (ex. Knight nachos)
                String[] mj = StdIn.readLine().split(" "); //reads line after dish for ingredient IDs and assigns to array
                int numOF = Integer.parseInt(mj[0]); //assigns first number of array which represents how many ingredient IDs there are 
                int[] masalaID = new int[numOF]; //creates integer array with size of number of ingredient IDs (ex. 9)

                for(int y = 0; y < numOF; y++) 
                {
                    masalaID[y] = Integer.parseInt(mj[y + 1]); //populates array at each index with each ingredient ID)
                }

                Dish plate = new Dish(categoryVar[i], n, masalaID); //creates new dish object with category name, dish name, and int array where each index is the ID of the ingredient needed for the dish 
                MenuNode firstNode = new MenuNode(plate, menuVar[i]); //creates new MenuNode with said dish at front of linked list
                menuVar[i] = firstNode; //assigns firstNode to front of linked list
            }
        }
        printRestaurant();
    ...
    public void addStockNode ( StockNode newNode ) {
        ...
        int i = newNode.getIngredient().getID() % stockVarSize;
    ...
    public void updateStock (String ingredientName, int ingredientID, int stockAmountToAdd) {
        ...
        int updated = node.getIngredient().getStockLevel() + stockAmountToAdd;
        node.getIngredient().setStockLevel(updated);
    ...
    public void updatePriceAndProfit() {
        ...
        for(int i = 0; i < menuVar.length; i++) 
        {
            for(MenuNode ptr = menuVar[i]; ptr != null; ptr = ptr.getNextMenuNode()) 
            {
                ...
                {
                    cost += findStockNode(ids[id]).getIngredient().getCost();
                }
                
                double price = cost * 1.2;
                ptr.getDish().setPriceOfDish(price);
                ptr.getDish().setProfit(price - cost);
            }
        }
    ...
    public void createStockHashTable (String inputFile){
        ...
        while(!StdIn.isEmpty()) 
        {
            ...
            Ingredient bron = new Ingredient(id, ingredient, stockNum, price);
            StockNode steph = new StockNode(bron, null);
            addStockNode(steph);
        }
        printRestaurant();
    ...
    public void addTransactionNode ( TransactionData data ) { // method adds new transactionNode to the end of LL
        ...
        TransactionNode bron = new TransactionNode(data, null);
    ...
    public boolean checkDishAvailability (String dishName, int numberOfDishes){
        ...
        int[] id = dish.getDish().getStockID();
        for(int i = 0; i < id.length; i++) 
        {
            StockNode node = findStockNode(id[i]);
            int stock = node.getIngredient().getStockLevel();
            if(stock < numberOfDishes)
            {
                return false;
            } 
        }
    ...
    public void order (String dishName, int quantity){
        ...
        int i = 0;
        MenuNode dish = findDish(dishName);
        String catName = dish.getDish().getCategory();
        for(int j = 0; j < categoryVar.length; j++)
        {
            if(catName.equals(categoryVar[j])) i = j;
        ...
        MenuNode steph = dish;
            do 
            {
                if(checkDishAvailability(bron.getDish().getName(), quantity) == true)
                {
                    double profit = bron.getDish().getProfit();
                    TransactionData mj = new TransactionData("order", bron.getDish().getName(), quantity, profit * quantity, true);
                    addTransactionNode(mj);
                    int[] stockID = bron.getDish().getStockID();
                    for(int a = 0; a < stockID.length; a++)
                    {
                        StockNode shaq = findStockNode(stockID[a]);
                        updateStock(shaq.getIngredient().getName(), shaq.getIngredient().getID(), -quantity);
                    }
                    break;
                }
                else
                {
                    TransactionData himothy = new TransactionData("order", bron.getDish().getName(), quantity, 0, false);
                    addTransactionNode(himothy);
                    if(bron.getNextMenuNode() == null)
                    {
                        dish = menuVar[i];
                        bron = dish;
                    }
    ...
    public double profit () {
        ...
        double bigBroBron = 0;
        for(TransactionNode bron = transactionVar; bron != null; bron = bron.getNext()) 
        {
            if(bron.getData().getSuccess()) 
            {
                bigBroBron += bron.getData().getProfit();
            }
        }
        
        return bigBroBron; // update the return value
    }
    ...
    public void donation (String ingredientName, int quantity){
        ...
        if(profit() > 50) 
        {
            ...
            if(node.getIngredient().getStockLevel() >= quantity) 
            {
                TransactionData bron = new TransactionData("donation", ingredientName, quantity, 0, true);
                addTransactionNode(bron);
                updateStock(ingredientName, node.getIngredient().getID(), -quantity);
            }
            else
            {
            TransactionData steph = new TransactionData("donation", ingredientName, quantity, 0, false);
            addTransactionNode(steph);
            }
        }
        else
        {
            TransactionData mj = new TransactionData("donation", ingredientName, quantity, 0, false);
            addTransactionNode(mj);
        }
    ...
    public void restock (String ingredientName, int quantity){
        ...
        StockNode nodeski = findStockNode(ingredientName);
        if(profit() > nodeski.getIngredient().getCost() * quantity) 
        {
            TransactionData bron = new TransactionData("restock", ingredientName, quantity, -(nodeski.getIngredient().getCost() * quantity), true);
            addTransactionNode(bron);
            updateStock(ingredientName, nodeski.getIngredient().getID(), quantity);
        }
        else
        {
            TransactionData steph = new TransactionData("restock", ingredientName, quantity, 0, false);
            addTransactionNode(steph);
        }
